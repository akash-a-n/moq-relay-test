...
folly::coro::Task<folly::Expected<std::shared_ptr<moxygen::Subscriber::PublishNamespaceHandle>, moxygen::PublishNamespaceError>>
...
MoxygenInterface::subscribe_namespace(const std::string &trackNamespace) {
  try {
    ...
    auto subscribeNamespaceResult = folly::coro::blockingWait(
        relaySession_->subscribeNamespace(subscribeNamespace, nullptr));

    if (!subscribeNamespaceResult.hasValue()) {
      auto error = subscribeNamespaceResult.error();
